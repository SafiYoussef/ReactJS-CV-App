...
require("http").get(
  ...
  function(response) {
    ...
    response.on("end", function() {
      ...
      if (!version) {
        console.log("Couldn't parse version");
        return process.exit(1);
      } else version = version[1];

      var packageVersion = version.replace(
        /\.\d$/,
        x => "." + (parseInt(x.substr(1)) + VERSION_INCREASE)
      );

      console.log(
        "Font Awesome v" + version + " (package version " + packageVersion + ")"
      );
      var package = require("fs")
      ...
      var namecount = 0;
      var main = body.match(
        /(\}\.fa-[a-z0-9\-]+(?::before,\.fa-[a-z0-9\-]+)*):before\{content:"\\([0-9a-f]+)"/g
      );
      if (!main) {
